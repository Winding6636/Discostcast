...
class URLPlaylistEntry(BasePlaylistEntry):
    ...
    async def _really_download(self) -> None:
        ...
        if self.playlist.bot.config.bgmmode:
            ...
            try:
                #await self.bgmmode(info.expected_filename)
                input_file = info.expected_filename
                ...
                output_file = os.path.splitext(input_file)[0]  + "_tmp" + ext
                ...
                if not ffmpeg_bin:
                    ...
                    return ""
                audio_filter = (
                    'dynaudnorm=p=0.1,'  # 音量正規化
                    'afade=t=in:st=0:d=10,'  # インフェード (開始から6秒)
                    f'afade=t=out:st={self.playlist.bot.config.bgmlength - 10}:d=10'  # アウトフェード (終了時間を設定)
                )
                ffmpeg_cmd = [
                    ffmpeg_bin,
                    "-i", input_file,
                    "-t", str(self.playlist.bot.config.bgmlength),
                    "-af", audio_filter,
                ...
                try:
                    ...
                    log.error(':: Exception while checking entry data. / ダウンロードもしくは音声処理に失敗しました。正しく再生されないかスキップされます。 ::')
                finally:
